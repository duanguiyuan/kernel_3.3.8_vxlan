				       ...
				       int min, int max, bool use_eth)
{
	...
	if (min >= max) {
		/* Use default range */
		/* 
			源码是 使用inet_get_local_port_range(net, &min, &max)
			后修改 inet_get_local_port_range(&min, &max)
			可能会出问题，调用此函数 时 打印信息 
			此函数的功能推测是  获取本地端口的 合法范围
		*/
		inet_get_local_port_range(&min, &max);
		printk("use inet_get_local_port_range maybe error !!! ");
	}
