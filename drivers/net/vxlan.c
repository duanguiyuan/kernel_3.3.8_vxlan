					...
					const u8 *mac)
{
	...
	struct vxlan_fdb *f;
/* tihuan ether_addr_equal函数和compare_ether_addr 是用来比较 mac是否相等的 */
		...
		hlist_for_each_entry_rcu_vxlan(f, &vxlan->fdb_head[h], hlist) {
			...
			list_for_each_entry_rcu(rd, &f->remotes, list) {
				err = vxlan_fdb_info(skb, vxlan, f,
					//tihuan	     NETLINK_CB(cb->skb).portid,
		      ...
		      struct sk_buff *skb, __be32 vx_vni)
{
	...
	vni = ntohl(vx_vni) >> 8;
	/* Is this VNI defined? */
	vxlan = vxlan_vs_find_vni(vs, vni);/* 匹配 vxlan ID  VNI 推测是 vxlan number id*/
	if (!vxlan)
		goto drop;
...
/* 自定义修改: 如果是 ipv6 的包 则丢弃 后期如果支持ipv6 应重新开启*/
#if 0
		oip6 = ipv6_hdr(skb);
	...
	if ((vxlan->flags & VXLAN_F_LEARN) &&
	    vxlan_snoop(skb->dev, &saddr, eth_hdr(skb)->h_source))/* vxlan fdb表学习 记录 mac与ip的关系*/
		goto drop;
...
/* tihuan 替换 mac 比较函数 兼容 不通版本的 内核  */
	//	diff = !ether_addr_equal(eth_hdr(skb)->h_dest, n->ha);
		diff = !(0 == compare_ether_addr(eth_hdr(skb)->h_dest, n->ha));
		if (diff) {
		   ...
		   __be16 src_port, __be16 dst_port, __be32 vni, bool xnet)
{
	...
	int min_headroom;
	int err;
// tihuan	bool udp_sum = !vs->sock->sk->sk_no_check_tx;
	bool udp_sum = !vs->sock->sk->sk_no_check;
...
//printk(" vxlan_setup ip sa_family :[%d] \n",vxlan->default_dst.remote_ip.sa.sa_family);
	...
	dev->netdev_ops = &vxlan_netdev_ops;	/* 设置 dev 操作结构体 */
	dev->destructor = free_netdev;			/* 设置 删除操作 */
	SET_NETDEV_DEVTYPE(dev, &vxlan_type);	/* 设置 网络设备的 类型 为 vxlan*/
	...
	dev->features |= NETIF_F_HW_VLAN_TX | NETIF_F_HW_VLAN_STAG_TX;

	dev->hw_features |= NETIF_F_SG | NETIF_F_HW_CSUM | NETIF_F_RXCSUM;
	dev->hw_features |= NETIF_F_GSO_SOFTWARE;

	dev->hw_features |= NETIF_F_HW_VLAN_TX | NETIF_F_HW_VLAN_STAG_TX;
